...
const Celebi = () => {
  ...
  const Subnames = () => (
    <h6 className="text-lg tracking-wider uppercase">
      {data.jsonld['@graph'][1].givenName[1]
        ? ' (' +
          data.jsonld['@graph'][1].givenName
            .map(function (elem, index) {
              if (index > 0 && elem['@value']) {
                let name = elem['@value']
                let familynameIndex = data.jsonld[
                  '@graph'
                ][1].familyName.findIndex(
                  (x) => x['@language'] === elem['@language'],
                )
                if (familynameIndex >= 0) {
                  if (
                    data.jsonld['@graph'][1].familyName[familynameIndex] &&
                    data.jsonld['@graph'][1].familyName[familynameIndex][
                      '@value'
                    ]
                  ) {
  ...
  const Names = () => (
    ...
    >
      {Array.isArray(data.jsonld['@graph'][1].givenName)
        ? data.jsonld['@graph'][1].givenName[0]['@value']
        : data.jsonld['@graph'][1].givenName}{' '}
      {Array.isArray(data.jsonld['@graph'][1].familyName)
        ? data.jsonld['@graph'][1].familyName[0]['@value']
        : data.jsonld['@graph'][1].familyName}
    </h1>
  )
        ...
        {_.get(data, 'jsonld["@graph"][1].seeks', []).map((x, index) => (
          <div key={'holder_' + index}>
            <p className="text-xs text-gray-800" key={'p_' + index}>
              {_.get(x, 'availableAtOrFrom.address.addressCountry', null) ||
              _.get(x, 'availableAtOrFrom.address.addressRegion', null) ||
              _.get(x, 'availableAtOrFrom.address.addressLocality', null)
                ? ''
                : ''}
              {(_.get(x, 'availableAtOrFrom.address.addressLocality', null)
                ? _.get(x, 'availableAtOrFrom.address.addressLocality', '') +
                  ' '
                : '') +
                (_.get(x, 'availableAtOrFrom.address.addressRegion', null)
                  ? _.get(x, 'availableAtOrFrom.address.addressRegion', '') +
                    ' '
                  : '') +
                (_.get(x, 'availableAtOrFrom.address.addressCountry', null)
                  ? _.get(x, 'availableAtOrFrom.address.addressCountry', '')
                  : '')}{' '}
              |{' '}
              {_.get(x, 'availabilityStarts', null)
                ? _.get(x, 'availabilityStarts', '')
                : ''}{' '}
              {_.get(x, 'availabilityEnds', null)
                ? '- ' + _.get(x, 'availabilityEnds', '')
                : ''}
            </p>
          </div>
        ))}
  ...
  const Address = () =>
    (data.jsonld['@graph'][1].address &&
      data.jsonld['@graph'][1].address.length > 0 &&
      data.address.enable && (
        <div className="mb-6">
          {data.jsonld['@graph'][1].address
            .filter(
    ...
    data.contacts.enable && (
      <div className="mb-6">
        ...
        <ContactItem
          label="Phone"
          value={_.get(
            _.find(data.jsonld['@graph'][1].contactPoint, {
    ...
    <div key={_.get(x, '@id', 'main')} className="my-3 mr-10">
      <div>
        ...
        <p className="text-xs text-gray-800">
          {_.get(x, 'roleName', '')} | {_.get(x, 'startDate', '')} -{' '}
          {_.get(x, 'endDate', '')}
        </p>
      </div>
      <ReactMarkdown
        className="mt-2 text-sm"
        source={_.get(x, 'description', '')}
      />
      <WorkResponsibility
        responsibilities={_.get(x, 'hasOccupation.responsibilities', [])}
      />
  ...
  const Work = () =>
    _.get(data, "jsonld['@graph'][1].hasOccupation", []).length &&
    data.work.enable && (
      <div className="mb-6">
        <Heading title={data.work.heading} />
        {_.get(data, "jsonld['@graph'][1].hasOccupation", [])
  ...
  const Education = () =>
    ...
    data.education.enable && (
      <div className="mb-6">
        ...
        {_.get(data, "jsonld['@graph'][1].hasCredential", [])
          .filter(
            (x) =>
  ...
  const userSkills = () => {
    let workSkills = _.chain(
      _.get(data, "jsonld['@graph'][1].hasOccupation", []),
    )
      .map('skills')
      .flatten()

    let awardSkills = _.chain(_.get(data, "jsonld['@graph'][0].award", []))
      .map('skill:assesses')
      .flatten()
    ...
    let interactionTeachSkills = _.chain(
      _.get(data, "jsonld['@graph'][1].interactionStatistic", []),
    )
      .map('result')
      .flatten()
      .map('teaches')
  ...
  const Languages = () =>
    ...
    _.get(data, 'jsonld["@graph"][1].knowsLanguage', []).length > 0 && (
      <div className="w-3/4 mx-auto mb-6">
        <Heading title={data.languages.heading} />
        <div>
          {_.get(data, 'jsonld["@graph"][1].knowsLanguage', [])
  ...
  const Awards = () =>
    ...
    data.awards.enable && (
      <div className="mb-6">
        ...
        {_.get(data.jsonld['@graph'][0], 'award', [])
  ...
  const Certifications = () =>
    ...
    data.certifications.enable && (
      <div className="mb-6">
        <Heading
          title={data.certifications.heading}
  ...
  const Extras = () =>
    ...
    data.extras.enable && (
      <div className="mb-6">
        ...
        {_.get(data.jsonld['@graph'][1], 'identifier', [])
          .filter((x) => x['value'] !== '')
          .map(ExtraItem)}
      </div>
    )
