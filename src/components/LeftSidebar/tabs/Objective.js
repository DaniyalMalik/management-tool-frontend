...
const ObjectiveTab = ({ data, onChange }) => {
  ...
  return (
    <>
      ...
      {data.jsonld['@graph'][1].seeks &&
        data.jsonld['@graph'][1].seeks.map((x, index) => (
          <Item
            dispatch={dispatch}
            first={index === 0}
            index={index}
      ...
      <AddItem
        ...
        dispatch={dispatch}
        size={_.size(data.jsonld['@graph'][1].seeks)}
      />
    </>
  )
}
  ...
  return (
    <div>
      ...
      <Availablity
        ...
        onChange={(key, value) =>
          addAvailability(value, key, item.availableAtOrFrom)
        }
      />
    </div>
  )
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  ...
  return (
    <div className="my-4 border border-gray-200 rounded p-5">
      <ItemHeading
        title={
          item.availableAtOrFrom.address.addressCountry ||
